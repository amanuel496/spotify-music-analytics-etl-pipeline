...
def validate_datasets_fn():
    ...
    try:
        ...
        if not missing_columns:
            ...
            logging.ingo("All required columns are present in songs dataset.")
        else:
            validation_results['songs'] = False
            logging.warning(f"Missing columns in songs dataset: {missing_columns}")
    except Exception as e:
        validation_results['songs'] = False
        logging.error(f"Failed to read or validate songs dataset from S3: {str(e)}") 
...
def move_processed_files():
    ...
    try:
        ...
        for file in stream_files:
            ...
            destination_key = file.replace('spotify_data/streams/', ARCHIVE_PREFIX)
...
with DAG('data_validation_and_kpi_computation', default_args=default_args, schedule='@daily') as dag:
    validate_datasets = PythonOperator(
        ...
        python_callable=validate_datasets_fn
    )
    ...
    calculate_genre_level_kpis >> calculate_hourly_kpis >> move_files >> end_dag